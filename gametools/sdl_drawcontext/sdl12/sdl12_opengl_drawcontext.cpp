...
SDL12_SurfaceRawImage::SDL12_SurfaceRawImage(SDL_Surface *s)
    : m_s(s)
{
    SDL_LockSurface(s);
    GLint  nOfColors;
...
SDL12_SurfaceRawImage::~SDL12_SurfaceRawImage()
{
    SDL_FreeSurface(m_s);
}
...
unsigned short * SDL12_OpenGLBackendUtil::utf8ToUnicode(const char *utf8Text,
...
OpenGLRawImage * SDL12_OpenGLBackendUtil::loadImage(ImageType type, const char *path)
{
...
//#ifdef DISABLED
    SDL_PixelFormat sdlformat = *(tmpsurf->format);
    switch (type) {
    case IMAGE_RGB:
        sdlformat.BytesPerPixel = 3;
        sdlformat.BitsPerPixel = 24;
        sdlformat.Rmask = 0x000000ff;
        sdlformat.Gmask = 0x0000ff00;
        sdlformat.Bmask = 0x00ff0000;
        sdlformat.Amask = 0xff000000;
        break;
    case IMAGE_RGBA:
    default:
        sdlformat.BytesPerPixel = 4;
        sdlformat.BitsPerPixel = 32;
        sdlformat.Rmask = 0x000000ff;
        sdlformat.Gmask = 0x0000ff00;
        sdlformat.Bmask = 0x00ff0000;
        sdlformat.Amask = 0xff000000;
    }
    retsurf = SDL_ConvertSurface(tmpsurf, &sdlformat, 0);
    if (retsurf==NULL) {
    ...
    SDL_FreeSurface (tmpsurf);
//#endif
    return new SDL12_SurfaceRawImage(retsurf);
}
...
OpenGLRawImage * SDL12_OpenGLBackendUtil::loadImagePNG(ImageType type, const char *path)
{
...
OpenGLRawImage * SDL12_OpenGLBackendUtil::loadImageJPG(ImageType type, const char *path)
{
