...
cd "${BUILD_DIRECTORY}"
${SOURCE_DIRECTORY}/configure -disable-dependency-tracking CXXFLAGS="${ARCHFLAGS}" CFLAGS="${ARCHFLAGS}" OBJCFLAGS="${ARCHFLAGS}" SDL_LIBS="${SDL_LIBS}" SDL_CFLAGS="${SDL_CFLAGS}" SDL_VERSION="${SDL_VERSION}" ADDITIONAL_FRAMEWORKS_DIRECTORY="${ADDITIONAL_FRAMEWORKS_DIRECTORY}" ADDITIONAL_FRAMEWORKS="${ADDITIONAL_FRAMEWORKS}"
make -j3
make -j3 mac-bundle
