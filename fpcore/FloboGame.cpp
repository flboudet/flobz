...
void FloboLocalGame::InitGame(RandomSystem *attachedRandom)
{
    ...
    this->_attachedRandom = _attachedRandom;
    _sequenceNr = 0;
    _semiMove = 0;
    _neutralFlobos = 0;
...
{
    // _unmoveableFlobo and _floboVector objects are cleaned up automatically by shared_ptr.
}
...
{
  if (_floboCells[X + Y * FLOBOBAN_DIMX])
    _floboCells[X + Y * FLOBOBAN_DIMX]->setFloboState(value);
};
...
void FloboLocalGame::cycleEnding()
{
    ...
    if (score >= 4) {
#ifdef DESACTIVE
        audio_sound_play(sound_splash[_phase>7?7:_phase]);
#endif
        score -= 3;
        if (_phase > 0) {
