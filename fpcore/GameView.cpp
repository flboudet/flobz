...
{
	_attachedGame = attachedFloboGameFactory->createFloboGame(&_attachedFloboFactory);
    _scoreDisplay.reset(new PlayerGameStatDisplay(_attachedGame->getGameStat()));
    _attachedGame->addGameListener(this);
	_gameRunning = true;
	_enemyGame = NULL;
    _skippedCycle = false;
    _cycleAllowance = 0;
...
int GameView::getValenceForFlobo(Flobo *flobo) const
{
    ...
    auto down  = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(i, j+1));
    auto right = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(i+1, j));
    auto up    = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(i, j-1));
    auto left  = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(i-1, j));
...
void GameView::cycleAnimation(void)
{
    ...
    if (_attachedGame->isEndOfCycle() && _attachedGame->isGameRunning() && !_newMetaCycleStart) cycleGame();
}
...
{
    if (cycleAllowed()) _attachedGame->moveLeft();
}

void GameView::moveRight()
{
    if (cycleAllowed()) _attachedGame->moveRight();
}

void GameView::rotateLeft()
{
    if (cycleAllowed()) _attachedGame->rotate(true);
}

void GameView::rotateRight()
{
    if (cycleAllowed()) _attachedGame->rotate(false);
}
...
void GameView::render(DrawTarget *dt)
{
    ...
    if (_showNextFlobos) {
        ...
        const FloboTheme &nextFloboTheme =
            _attachedThemeSet->getFloboTheme(_attachedGame->getNextFalling());
        ...
        const FloboTheme &nextCompanionTheme =
            _attachedThemeSet->getFloboTheme(_attachedGame->getNextCompanion());
...
void GameView::renderNeutral(DrawTarget *dt)
{
    if (!_haveDisplay) return;
	IosRect drect;
    int neutralFlobos = _attachedGame->getNeutralFlobos();
...
void GameView::gameDidAddNeutral(std::shared_ptr<Flobo> neutralFlobo, int neutralIndex, int totalNeutral) {
    ...
    for (int i = y ; i < FLOBOBAN_DIMY ; i++) {
        auto belowFlobo = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(x, i));
        if (belowFlobo != nullptr) {
            belowFlobo->addAnimation(std::make_shared<SmoothBounceAnimation>(belowFlobo, synchronizer));
        }
    }
...
{
    if (!_haveDisplay) return;
    auto animatedFallingFlobo = std::static_pointer_cast<AnimatedFlobo>(fallingFlobo);
    auto animatedCompanionFlobo = std::static_pointer_cast<AnimatedFlobo>(companionFlobo);
    animatedFallingFlobo->flushAnimations(ANIMATION_H);
    animatedCompanionFlobo->flushAnimations(ANIMATION_H);
    animatedFallingFlobo->addAnimation(std::make_shared<MovingHAnimation>(animatedFallingFlobo, TSIZE, 4));
    animatedCompanionFlobo->addAnimation(std::make_shared<MovingHAnimation>(animatedCompanionFlobo, TSIZE, 4));
}
...
void GameView::fallingsDidFallingStep(std::shared_ptr<Flobo> fallingFlobo, std::shared_ptr<Flobo> companionFlobo)
{
    ...
    auto animatedFallingFlobo = std::static_pointer_cast<AnimatedFlobo>(fallingFlobo);
    auto animatedCompanionFlobo = std::static_pointer_cast<AnimatedFlobo>(companionFlobo);
    animatedFallingFlobo->flushAnimations(ANIMATION_V);
    animatedCompanionFlobo->flushAnimations(ANIMATION_V);
    animatedFallingFlobo->addAnimation(std::make_shared<MovingVAnimation>(animatedFallingFlobo, -TSIZE/2, 4));
    animatedCompanionFlobo->addAnimation(std::make_shared<MovingVAnimation>(animatedCompanionFlobo, -TSIZE/2, 4));
}
...
{
    if (!_haveDisplay) return;
    // Just to be sure of what we get if data comes from network
    if ((companionFlobo == nullptr) || (fallingFlobo == nullptr))
        return;
    auto animatedCompanionFlobo = std::static_pointer_cast<AnimatedFlobo>(companionFlobo);
    auto animatedFallingFlobo = std::static_pointer_cast<AnimatedFlobo>(fallingFlobo);
    animatedCompanionFlobo->setPartner(animatedFallingFlobo);
    animatedCompanionFlobo->flushAnimations(ANIMATION_ROTATE);
    animatedCompanionFlobo->addAnimation(std::make_shared<TurningAnimation>(animatedCompanionFlobo, counterclockwise));
}

void GameView::floboDidFall(std::shared_ptr<Flobo> flobo, int originX, int originY, int nFalledBelow)
{
    if (!_haveDisplay) return;
    auto animatedFlobo = std::static_pointer_cast<AnimatedFlobo>(flobo);
    animatedFlobo->flushAnimations();
    animatedFlobo->addAnimation(std::make_shared<FallingAnimation>(animatedFlobo, originY, _xOffset, _yOffset, nFalledBelow));
}
...
void GameView::floboWillVanish(std::vector<std::shared_ptr<Flobo>> &floboGroup, int groupNum, int phase)
{
    ...
    for (int i = 0, j = floboGroup.size() ; i < j ; i++) {
        ...
        if (currentFlobo->getFloboState() != FLOBO_NEUTRAL)
            newAnimation = std::make_shared<VanishAnimation>(currentFlobo, i*2 , _xOffset, _yOffset, synchronizer, i, floboGroup.size(), groupNum, phase);
        else
            newAnimation = std::make_shared<NeutralPopAnimation>(currentFlobo, i*2, synchronizer);
...
void GameView::gameDidEndCycle()
{
	if (_enemyGame != NULL) {
		if (_attachedGame->getNeutralFlobos() < 0)
			_enemyGame->increaseNeutralFlobos(- _attachedGame->getNeutralFlobos());
	}
...
{
    _attachedGame->getGameStat().is_winner = true;
    _gameRunning = false;
}

void GameView::gameLost()
{
    _attachedGame->getGameStat().is_winner = false;
    _gameRunning = false;
    if (!_haveDisplay) return;
    for (int i = 0 ; i <= FLOBOBAN_DIMX ; i++) {
        for (int j = 0 ; j <= FLOBOBAN_DIMY ; j++) {
            if (_attachedGame->getFloboAt(i, j) != nullptr) {
                auto currentFlobo = std::static_pointer_cast<AnimatedFlobo>(_attachedGame->getFloboAt(i, j));
                currentFlobo->addAnimation(std::make_shared<GameOverFallAnimation>(currentFlobo, (j - FLOBOBAN_DIMY) + abs((FLOBOBAN_DIMX / 2) - i) * 5));
            }
        }
    }
